...
- **`container` CLI discovery**: `Process.findExecutablePath()` in `Sources/orchard/Extensions/Process+Extensions.swift` searches common install paths (homebrew, rancher, orbstack, `~/.local/bin`), then `$PATH`, then falls back to `zsh -lc 'which container'`. Two env vars override this for tests: `ORCHARD_CONTAINER_PATH=<path>` forces a specific binary; `ORCHARD_FORCE_NO_CONTAINER=1` simulates a missing binary. Use these — don't stub `Process` itself.

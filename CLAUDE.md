...
- **`container` CLI discovery**: `ContainerProcessService` in `Sources/orchard/Services/ContainerProcessService.swift` searches common install paths (homebrew, rancher, orbstack, `~/.local/bin`), then `$PATH`, then falls back to `zsh -lc 'which container'`. The discovered path is cached for the lifetime of the service; `ContainerProcessService.shared` is the process-wide cached instance used by default. One env var overrides this for tests and is evaluated on every call (never cached): `ORCHARD_CONTAINER_PATH=<path>` forces a specific binary. Use this — don't stub `Process` itself.
